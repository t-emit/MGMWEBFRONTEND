            ...
            <div className="container mx-auto px-4 max-w-7xl relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="relative h-96 overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-100">
                             {eventsOnly.length > 0 ? (
                                <div className="absolute inset-0 events-scroll-container"><div className="events-scroll-content animate-scrollUp">
                                    {duplicatedEvents.map((item, index) => (
                                        <div key={`${item._id}-event-${index}`} className="group p-6 hover:bg-amber-50 transition-all duration-300 border-b border-gray-100"><div className="flex items-start gap-4">
                                             <div className="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-amber-500 to-orange-500 rounded-lg flex items-center justify-center text-white font-bold text-sm text-center p-2">{new Date(item.date).toLocaleDateString('en-US', { day: 'numeric', month: 'short' })}</div>
                                            <div className="flex-1">
                                                <span className="text-xs text-amber-600 uppercase tracking-wide font-semibold">Event</span>
                                                <h4 className="font-bold text-gray-800 group-hover:text-amber-700 transition-colors mb-2 line-clamp-2">{item.title}</h4>
                                                <p className="text-gray-600 text-sm mb-3 line-clamp-2">{item.description}</p>
                                                <a href={item.link} className="inline-flex items-center gap-1 text-amber-600 hover:text-amber-800 font-semibold text-sm transition-colors group/link">View Details <svg className="w-4 h-4 transform group-hover/link:translate-x-1 transition-transform" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L12.586 11H5a1 1 0 110-2h7.586l-2.293-2.293a1 1 0 010-1.414z" clipRule="evenodd" /></svg></a>
                                            </div>
                                        </div></div>
                                    ))}
                                </div></div>
