// src/pages/Academic/Entc/EntcLaboratoryDetailPage.jsx
import React, { useEffect, useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import { entcLaboratories, allEntcFacultyData } from './entcConstants'; // Import E&TC lab and faculty data

const EntcLaboratoryDetailPage = () => {
  const { labId } = useParams(); // Get labId from URL (e.g., 'digital-signal-processing-lab')
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    const foundLab = entcLaboratories.find(lab => lab.id === labId);
  ...
  const inchargeFaculty = labDetails.incharge ? allEntcFacultyData.find(f => f.name === labDetails.incharge) : null;
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 mb-10 border border-gray-100 print:shadow-none print:border-none print:rounded-none">
        ...
        <div className="flex flex-col md:flex-row items-start md:space-x-8 print:flex-col print:space-x-0">
          ...
          <div className="md:w-1/2 flex-shrink-0 mt-6 md:mt-0 print:w-full print:mt-4">
            ...
            <img
              src={`/images/entc/labs/${labDetails.id}.jpg`} // Assuming lab-specific images
              alt={labDetails.name}
...
export default EntcLaboratoryDetailPage;